...
class AddEditServerViewModel(
    ...
    fun testConnection(serverConfig: ServerConfig) {
        ...
        val job = viewModelScope.launch {
            if (serverConfig.isGitHubActionsServer && serverConfig.gitHubConfig != null) {
                val res = gitHubApiClient.validateConnection(
                    token = serverConfig.gitHubConfig.token,
                    owner = serverConfig.gitHubConfig.owner,
                    repo = serverConfig.gitHubConfig.repo
                )
                if (res.isSuccess) {
                    eventChannel.send(Event.TestSuccess)
                } else {
                    eventChannel.send(Event.TestFailure(RequestResult.Error.RequestError.Unknown(res.exceptionOrNull()?.message ?: "GitHub error")))
                }
                return@launch
            }
