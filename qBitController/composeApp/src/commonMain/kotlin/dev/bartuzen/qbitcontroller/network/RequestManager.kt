...
class RequestManager(
    ...
    private suspend fun updateVersionIfNeeded(serverId: Int) {
        ...
        if (serverConfig != null && serverConfig.isGitHubActionsServer) {
            versions[serverId] = Clock.System.now() to QBittorrentVersion(5, 0, 0)
            return
        }
    ...
    private suspend fun tryLogin(serverId: Int): RequestResult<Unit> {
        ...
        val service = getTorrentService(serverId)
        if (serverConfig == null) return RequestResult.Success(Unit)
    ...
    suspend fun <T : Any> request(serverId: Int, block: suspend (service: TorrentService) -> Response<T>): RequestResult<T> {
        val serverConfig = serverManager.getServerOrNull(serverId)
        if (serverConfig != null && (serverConfig.isGitHubActionsServer || serverConfig.url.contains("github.com") || serverConfig.gitHubConfig != null)) {
            return RequestResult.Error.RequestError.NoData
        }
